...
| Layer          | Stack |
|----------------|--------|
| **Backend**    | Python, Django, Django REST Framework, PostgreSQL, JWT (djangorestframework-simplejwt) |
| **Database**   | PostgreSQL |
...
Development is **stage-based**. Stages 1, 2, and 3 (3.1–3.5) are **completed**.
...
### Stage 3 — Users and Roles (completed)

- **3.1** — Custom user model (email login)
- **3.2** — Role system (superadmin, admin, realtor)
- **3.3** — Authentication (JWT, djangorestframework-simplejwt)
- **3.4** — Staff profiles (RealtorProfile)
- **3.5** — Agency model and admin
...
- Role system (superadmin, admin, realtor)
- JWT authentication
- Current user endpoint (`GET /api/auth/me/`)
- Realtor staff profiles
- Agency model
...
|----------|--------|-------------|
| `/api/auth/login/` | POST | Body: `{"email","password"}`. Returns `access` and `refresh` tokens. |

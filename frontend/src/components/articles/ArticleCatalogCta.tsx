...
/**
 * Dark catalog CTA card at the end of the article column (replaces the old
 * ArticleCatalogLinksBlock). Primary button opens /catalog; the ghost pills
 * are the deterministic SEO-landing links from lib/articleCatalogLinks.ts —
 * the first entry there IS /catalog, which the button already covers, hence
 * slice(1).
 */
export function ArticleCatalogCta() {
  const pills = ARTICLE_PAGE_CATALOG_LINKS.slice(1);
    ...
    <section className="relative mt-11 overflow-hidden rounded-[20px] bg-surface-dark p-7 font-sans text-white md:p-11">
      <div className="relative z-[1]">
        ...
        <p className="mt-3 max-w-[480px] text-[15px] leading-[1.6] text-white/70">
          Квартиры, дома и участки в Краснодаре и Геленджике — с проверкой
          документов и сопровождением сделки.
        </p>
        <Link
          href="/catalog"
